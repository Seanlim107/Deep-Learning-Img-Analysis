...
# Backbone name
backbone_model_name = 'BaselineCNN_ckpt'
...
def plot_embeds(train_settings, model, dataloader, epoch, mode='Training', logger=None, ):
    ...
    loss_func1 = Contrastive_Loss(margin=train_settings['margin'])
    
    # Evaluate predictions with true outputs
    total_loss=0
    with torch.no_grad():
        for iter, (X1, X2, simi, y1) in enumerate(dataloader):
            X1, X2, simi = X1.to(device), X2.to(device), simi.to(device)
            ypred1, ypred2 = model(X1, X2)
            labels.append(y1.cpu().detach().numpy())
            embeddings.append(ypred1.cpu().detach().numpy())
            loss = loss_func1(ypred1, ypred2, simi.long())
            total_loss+=loss
        ...
        logger.log({f'Contrastive_Backbone_Fixed_{mode}_Loss': total_loss/len(dataloader)})
        ...
        directory='plot_images_Backbone_Fixed'
...
def evaluate(data_settings, model, dataloader, epoch, mode='Training', logger=None, ):
    model.eval()
    with torch.no_grad():
...
def train(data_settings, model_settings, train_settings):
    ...
    backbonefile = f"{backbone_model_name}.pth"
    ...
    wandb_logger = Logger(f"inm705_Baseline_Contrastive_w_Backbone_Fixed", project='inm705_CW')
    ...
    plot_embeds(train_settings, contrastive_model, asl_train_testloader, epoch=ckpt_epoch, mode='Training', logger=logger)
    plot_embeds(train_settings, contrastive_model, asl_testloader, epoch=ckpt_epoch, mode='Testing', logger=logger)
    plot_embeds(train_settings, contrastive_model, asl_validloader, epoch=ckpt_epoch, mode='Validation', logger=logger)
    ...
    for epoch in range(ckpt_epoch+1, train_settings['epochs']+1):
        ...
        logger.log({'train_loss_Contrastive_Backbone': total_loss/len(asl_trainloader)})
        
        plot_embeds(train_settings, contrastive_model, asl_train_testloader, epoch=epoch, mode='Training', logger=logger)
        plot_embeds(train_settings, contrastive_model, asl_testloader, epoch=epoch, mode='Testing', logger=logger)
        plot_embeds(train_settings, contrastive_model, asl_validloader, epoch=epoch, mode='Validation', logger=logger)
