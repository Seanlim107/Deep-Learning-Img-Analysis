...
def plot_embeds(train_settings, model, dataloader, epoch, mode='Training', logger=None, ):
    ...
    with torch.no_grad():
        ...
        # plt.show()
        directory='plot_images_ZS_Backbone'
...
def train(data_settings, model_settings, train_settings):
    ...
    include_others = True
    asl_dataset = ASL_Dataset_Contrastive(mode='train', img_size=data_settings['img_size'], include_others=include_others, simi_ratio=data_settings['simi_ratio'])
    ...
    if os.path.isfile(backbonefile):
        ...
        print('No Backbone, starting from scratch')
    ...
    for epoch in range(ckpt_epoch+1, train_settings['epochs']+1):
        ...
        if min_loss == -1:
            save_checkpoint_ZS(epoch, contrastive_model, cont_model_name, optimizer)
            min_loss = total_loss
