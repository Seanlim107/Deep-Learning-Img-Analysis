    ...
    with torch.no_grad():
        ...
        for iter,label in enumerate(np.unique(labels)):
            ...
            if label<26:
                label_to_plot = 0
                if(iter==0):
                    ax.scatter(to_plot_pca[class_indices,0], to_plot_pca[class_indices,1], zs=to_plot_pca[class_indices,2], zdir='z', label=label_to_plot, c=[cmap(label_to_plot)])
                else:
                    ax.scatter(to_plot_pca[class_indices,0], to_plot_pca[class_indices,1], zs=to_plot_pca[class_indices,2], zdir='z', c=[cmap(label_to_plot)])
            else:
                temp_labels+=1
